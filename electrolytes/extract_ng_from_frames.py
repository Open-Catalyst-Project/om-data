...
for st in st_list:
    for an in anions:
        for an_idx in evaluate_asl(st, f'at.ele {an}'):
            st.atom[an_idx].formal_charge = -1
    for cat in cations:
        for cat_idx in evaluate_asl(st, f'at.ele {cat}'):
            st.atom[cat_idx].formal_charge = 1
...
def sulfur_to_se(st):
    S_idx = evaluate_asl(cluster, f'smarts. S=P and at.ele S')
    if S_idx:
        st_copy = st.copy()
        for at in S_idx:
...
def parallel_work(data):
    ...
    extracts = set()
    for ng in NGS:
        for ng_idx in evaluate_asl(st, f'at.ele {ng}'):
            at_idxs = evaluate_asl(st, f'fillres (within 6 at.num {ng_idx})')
            if len(at_idxs) == 1:
                continue
    ...
    for idx, at_idxs in enumerate(extracts):
        ...
        if set(at.element for at in cluster.atom).issubset(NGS):
            continue
