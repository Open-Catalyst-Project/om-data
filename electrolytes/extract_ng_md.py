...
from schrodinger.structure import _Residue as Residue
...
def generate_frames(dir_name):
    fname = glob.glob(os.path.join(dir_name, "*-out.cms"))[0]
...
def main():
    args = parse_args()
    dir_list = glob.glob(os.path.join(args.output_path, "*crude"))
    print(dir_list)
