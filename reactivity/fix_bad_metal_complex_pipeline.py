...
#    os.makedirs(args.output_path)
    ...
    for ii in tqdm(range(args.start_index, args.end_index)):
        ...
        if name in fix_charge_spin_dict.keys():
            if os.path.exists(os.path.join(args.output_path, name)):
                shutil.rmtree(os.path.join(args.output_path, name))
            old_charge = old_charge_spin_dict[name]['total_charge']
            old_uhf = old_charge_spin_dict[name]['total_uhf']
    ...
    print(f"Number of structures fixed: {num_fixed}")
